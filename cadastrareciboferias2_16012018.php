...
if(isset($_POST['done'])){   		  
	...
	$dt_recibo_ferias = $_POST['dt_recibo'];	
	$dias_dt_recibo_ferias = $_POST['dias_recibo_ferias'];	
...
if ($controle_folga_periodo <> "Selecione..."){
		...
		if(empty ($matricula)||empty ($dt_recibo_ferias)||empty ($dias_dt_recibo_ferias)){
...
$periodos_separados  = explode ("-",$periodo);
       ...
       $sql = mysql_query("update controle_ferias set dt_recibo_ferias2 = '$dt_recibo_ferias',dias_dt_recibo_ferias2='$dias_dt_recibo_ferias' where matricula= '$matricula' and per_inicial= '$periodos_separados[0]' and per_final= '$periodos_separados[1]' and tipo_registro='NORMAL' ") or die(mysql_error());
...
<font size=4><b><u>Cadastro de Recibo de Férias</font></b></u>

<form name="form1" action="cadastrareciboferias2_16012018.php" method="POST" style="padding-top:40px;">
...
if(isset($_POST['done'])){   
    $id_retorno = $_POST['usuario_consulta'];    
		   ...
		   $per_fruicao1 = @mysql_result($sqltudo, $j, "per_inicial");           
		   $per_fruicao_11 = @mysql_result($sqltudo, $j, "per_final");          
		   $matricula = @mysql_result($sqltudo, $j, "matricula");
		   
           if ($per_fruicao1 and $per_fruicao_11 <> ''){
            print'<option>'; 						
            echo $per_fruicao1;
            echo '-';
            echo $per_fruicao_11;
...
<tr>
<td>Data Recibo Ferias:</td>
<td><input type="text" value="<?php echo $dt_recibo;?>" name="dt_recibo" id="dt_recibo" maxlength="10" onkeypress="mascaraData( this, event )" /> dd/mm/aaaa</td> 
</tr>
<tr>
<td>Dias Recibo:</td>
<td><input type="text"  name="dias_recibo_ferias" id="dias_recibo_ferias" maxlength="2" /> </td> 
</tr>
<hr>
