...
<font size=4><b><u>Cadastro de Folgas</font></b></u>
 <?php
...
if ($controle_folga_periodo <> "Selecione..."){
		...
		if(empty ($matricula)||empty($dt_folga)||empty ($obs_folga)||empty ($dias_folga)){
...
$periodos_separados  = explode ("-",$periodo);
       ...
       $sql = mysql_query("INSERT INTO `controle_ferias`(`matricula`,`dt_folga`, `obs_folga`,`dias_folga`,`usuario`,`tipo_registro`,`ip`,`data_criacao`,`per_inicial`,`per_final`) VALUES ('$matricula','$dt_folga','$obs_folga','$dias_folga','$usuario','FOLGA','$ip',now(),'$periodos_separados[0]','$periodos_separados[1]')") or die(mysql_error());
...
<form name="form1" action="cadastrafolgas22062017.php" method="POST" style="padding-top:40px;">
...
if(isset($_POST['done'])){   
    ...
    $id_retorno = $_POST['usuario_consulta'];    
           ...
           $sqltudo = mysql_query("select  matricula,per_inicial,per_final FROM controle_ferias where matricula='$id_retorno' AND tipo_registro='NORMAL' ")  or die(mysql_error());
           $colunas = mysql_num_rows($sqltudo);		
...
<tr>
<td>Data Folga:</td>
<td><input type="text" name="dt_folga" id="dt_folga" maxlength="10" onkeypress="mascaraData( this, event )" /> dd/mm/aaaa</td> 
</tr>
<hr>
<tr>
<td>Dias de Folga:</td>
<td><select name="dias_folga" class="campo" id="dias_folga">
<option>Selecione...</option>
<option>1</option>
<option>0.5</option>
</select>
...
<tr>
<td>Obs:</td>
<td><textarea name="obs_folga" rows="10" cols="40" onkeyup="maiuscula(this)" ></textarea></td>
</tr>
<tr>
<br>
